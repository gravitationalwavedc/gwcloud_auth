...
class TestLigo(TestCase):
    def test_ligo_auth_user(self):
        ...
        username_hash = sha3_512((shibboleth_dict['uid'] + settings.SECRET_KEY).encode()).hexdigest()
    ...
    def test_ligo_auth_special(self):
        ...
        self.assertTrue(b'window.location = "https://special.com/callback/' in response.content)

        # Extract the token from the response
        loc = [line for line in response.content.splitlines() if b"window.location" in line][0]
        loc = loc.strip()
        token = loc.split(b"\"")[1]
