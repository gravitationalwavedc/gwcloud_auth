...
def register(args, project):
    ...
    if not r.json()["success"]:
        return False, [("captcha", ["Captcha was invalid"])]
    ...
    if form.is_valid():
        ...
        verification_link = (
            utility.get_absolute_site_url(project)
            + "/auth/verify/?code="
            + utility.get_token(
...
def ligo_auth(request):
    ...
    if "special" in request.GET:
        ...
        response_token = jwt.encode(
            {"token": token, "refresh_token": str(refresh_token), "next_url": next_url},
            settings.SECRET_KEY,
            algorithm="HS256",
        )
        ...
        return HttpResponseRedirect(
            parse.urljoin(domain, reverse("ligo_continue", args=[response_token]))
        )
